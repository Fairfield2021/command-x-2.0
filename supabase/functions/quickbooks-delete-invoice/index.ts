...
async function qbRequest(method: string, endpoint: string, accessToken: string, realmId: string, body?: any) {
  ...
  // For delete operations, QB may return empty body on success
...
serve(async (req) => {
  ...
  try {
    ...
    console.log("Deleting QuickBooks invoice for local invoice:", invoiceId, "by user:", authResult.userId);
    ...
    if (!mapping) {
      console.log("Invoice not synced to QuickBooks, nothing to delete");
      return new Response(
        JSON.stringify({ 
          success: true, 
    ...
    if (mapping.sync_status === 'deleted') {
      console.log("Invoice already deleted in QuickBooks");
      return new Response(
        JSON.stringify({ 
          success: true, 
          message: "Invoice already deleted in QuickBooks",
          deleted: true 
        }),
        { headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    ...
    try {
      ...
      // If invoice not found in QB (already deleted there), clean up mapping
      if (fetchError.message?.includes('404') || fetchError.message?.includes('not found')) {
        console.log("Invoice not found in QuickBooks, likely already deleted");
        ...
        return new Response(
          JSON.stringify({ 
            success: true, 
            message: "Invoice not found in QuickBooks (may already be deleted)",
            deleted: true 
          }),
          { headers: { ...corsHeaders, "Content-Type": "application/json" } }
        );
    ...
    // Check if invoice has payments (cannot delete if payments exist)
    if (qbInvoiceData.Invoice.Balance !== qbInvoiceData.Invoice.TotalAmt) {
      ...
      console.warn(`Invoice has payments (${paidAmount} paid), cannot delete directly`);
      ...
      await supabase.from("quickbooks_sync_logs").insert({
        ...
        details: { 
          ...
          error: `Invoice has payments (${paidAmount} paid). Delete payments in QuickBooks first.`
        },
      });
      ...
      return new Response(
        JSON.stringify({ 
          success: false, 
          error: `Cannot delete invoice with payments. The invoice has $${paidAmount.toFixed(2)} in payments. Please delete the payments in QuickBooks first.`,
          deleted: false 
        }),
    ...
    // Delete the invoice in QuickBooks
    // QuickBooks delete uses POST with operation=delete
    console.log("Deleting invoice in QuickBooks...");
    const deletePayload = {
      Id: qbInvoiceId,
      SyncToken: syncToken,
    };
    ...
    console.log("Invoice deleted successfully from QuickBooks");
    ...
    return new Response(
      JSON.stringify({ 
        success: true, 
        message: "Invoice deleted in QuickBooks",
    ...
    return new Response(
      JSON.stringify({ success: false, error: errorMessage, deleted: false }),
