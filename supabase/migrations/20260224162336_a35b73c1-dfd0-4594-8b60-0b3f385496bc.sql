...
BEGIN
  -- Determine the transaction date column safely using hstore
  v_txn_date := COALESCE(
    (hstore(NEW) -> 'due_date')::date,
    (hstore(NEW) -> 'bill_date')::date,
    (hstore(NEW) -> 'date')::date,
    (hstore(NEW) -> 'created_at')::date
  );
